...
public class GestureListener implements OnGestureListener, OnDoubleTapListener, OnScaleGestureListener {
    ...
    public GestureListener(Context context, CompanionView aView) {
        this.mCompanionView = aView;
        ...
        screenPPI = (int) ((metrics.xdpi + metrics.ydpi) / 2.0f);
    }
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (e2.getPointerCount() == 2) {
            int gesturePhase = 1;
            float locX = (e2.getX(0) + e2.getX(1)) / 2.0f;
            float locY = (e2.getY(0) + e2.getY(1)) / 2.0f;
            ...
            if (pid1 >= 0 && pid1 < 2 && pid2 >= 0 && pid2 < 2) {
                if (!this.mInPanTransformGesture) {
                    ...
                    if (this.mInRotateTransformGesture || Math.abs(absoluteRotation) > _FP_GESTURE_ROTATION_THRESHOLD_DEGREES) {
                        ...
                        this.mCompanionView.onGestureListener(gesturePhase, 2, true, locX, locY, 1.0f, 1.0f, rotation, 0.0f, 0.0f);
                        rotation = 0.0f;
                    }
                }
                if (!(this.mInZoomTransformGesture || this.mInRotateTransformGesture)) {
                    if (isPanGesture(this.mDownTouchPoints[pid1], this.mDownTouchPoints[pid2], currentTouchPoint[0], currentTouchPoint[1])) {
                        ...
                        this.mCompanionView.onGestureListener(gesturePhase, 1, true, locX, locY, 1.0f, 1.0f, rotation, offsetX, offsetY);
                    } else if (this.mInPanTransformGesture) {
                        endPanGesture();
                        setDownTouchPoint(currentTouchPoint[0].f4x, currentTouchPoint[0].f5y, currentTouchPoint[0].pid);
                        setDownTouchPoint(currentTouchPoint[1].f4x, currentTouchPoint[1].f5y, currentTouchPoint[1].pid);
                    }
                }
            }
            ...
            if (pid_swipe >= 0 && pid_swipe < 2 && this.mCheckForSwipe && e1.getPointerCount() == 1) {
                boolean isSwipe;
                float delta_x = e2.getX(0) - this.mDownTouchPoints[pid_swipe].f4x;
                float delta_y = e2.getY(0) - this.mDownTouchPoints[pid_swipe].f5y;
                ...
                if (isSwipe) {
                    this.mCompanionView.onGestureListener(8, 4, true, e1.getX(0), e2.getY(0), 1.0f, 1.0f, 0.0f, offsetX, offsetY);
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        this.mCompanionView.onGestureListener(gesturePhase, 0, true, locX, locY, scaleX, scaleY, 0.0f, 0.0f, 0.0f);
        return true;
    }
    ...
    private float getRotation(TouchPoint initial_pt1, TouchPoint initial_pt2, TouchPoint current_pt1, TouchPoint current_pt2) {
        ...
        return (float) (((Math.atan2((double) (current_pt2.f5y - current_pt1.f5y), (double) (current_pt2.f4x - current_pt1.f4x)) * 180.0d) / 3.141592653589793d) - ((Math.atan2((double) (initial_pt2.f5y - initial_pt1.f5y), (double) (initial_pt2.f4x - initial_pt1.f4x)) * 180.0d) / 3.141592653589793d));
    }

    private boolean isPanGesture(TouchPoint initial_pt1, TouchPoint initial_pt2, TouchPoint current_pt1, TouchPoint current_pt2) {
        float delta_pt1_x = current_pt1.f4x - initial_pt1.f4x;
        float delta_pt1_y = current_pt1.f5y - initial_pt1.f5y;
        float delta_pt2_x = current_pt2.f4x - initial_pt2.f4x;
        float delta_pt2_y = current_pt2.f5y - initial_pt2.f5y;
