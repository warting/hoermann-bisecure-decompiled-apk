package flashx.textLayout.conversion
{
   ...
   class BaseTextLayoutImporter extends ConverterBase implements ITextImporter
   {
      ...
      public function parseTableCellElementChildren(param1:XML, param2:FlowGroupElement, param3:Object = null, param4:Boolean = false) : void
      {
         ...
         for each(_loc6_ in param1.children())
         {
            ...
            {
               if(_loc6_.name().localName == "p")
               {
                  _loc5_ = new TextFlow();
                  this.parseObject(_loc6_.name().localName,_loc6_,_loc5_,param3);
               }
               else if(_loc6_.name().localName == "TextFlow")
               {
                  TableCellElement(param2).textFlow = this.createTextFlowFromXML(_loc6_);
               }
            }
            else if(_loc6_.nodeKind() == "text")
            {
               _loc7_ = _loc6_.toString();
               _loc8_ = false;
               if(param2 is ContainerFormattedElement)
               {
                  _loc8_ = _loc7_.search(anyPrintChar) == -1;
               }
               if(!_loc8_)
               {
                  _loc5_ = new TextFlow();
            ...
            if(_loc5_)
            {
               TableCellElement(param2).textFlow = _loc5_;
               _loc5_ = null;
            }
         }
      }
