package flashx.textLayout.compose
{
   ...
   public class StandardFlowComposer extends FlowComposerBase implements IFlowComposer
   {
      ...
      public function updateToController(param1:int = 2147483647) : Boolean
      {
         ...
         {
            _loc5_ = this.getControllerAt(0);
            if(_loc5_)
            {
               _loc2_ = _loc5_.container.height;
            }
         }
         var _loc3_:ISelectionManager = textFlow.interactionManager;
         if(_loc3_)
         {
            _loc3_.flushPendingOperations();
         }
         ...
         if(_loc2_ && _loc5_.container.height != _loc2_)
         {
            _loc6_ = (textFlow.parentElement as TableCellElement).getTable();
            _loc6_.modelChanged(ModelChange.ELEMENT_MODIFIED,_loc6_,0,_loc6_.textLength);
            _loc6_.getTextFlow().flowComposer.updateAllControllers();
            if(_loc3_ && _loc3_.focused)
            {
               _loc5_.setFocus();
            }
         }
      ...
      override public function isPotentiallyDamaged(param1:int) : Boolean
      {
         var _loc2_:ContainerController = null;
         if(!super.isPotentiallyDamaged(param1))
         {
