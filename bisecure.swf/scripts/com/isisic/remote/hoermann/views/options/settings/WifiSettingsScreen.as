package com.isisic.remote.hoermann.views.options.settings
{
   ...
   import com.isisic.remote.hoermann.components.overlays.screens.WifiSettingsScreenOverlay;
   import com.isisic.remote.hoermann.components.popups.SelectBox;
   ...
   public class WifiSettingsScreen extends View implements IBindingClient
   {
      ...
      public function WifiSettingsScreen()
      {
         ...
         {
            watcherSetupUtilClass = getDefinitionByName("_com_isisic_remote_hoermann_views_options_settings_WifiSettingsScreenWatcherSetupUtil");
      ...
      private function dispose() : void
      {
         ...
         {
            HoermannRemote.appData.activeConnection.processor.removeEventListener(MCPEvent.RECEIVED,this.onMCP);
         }
      }
      ...
      private function loadNetworkState() : void
      {
         ...
         loader = Logicware.initMCPLoader(HoermannRemote.appData.activeConnection,loaderComplete = function(param1:Event):void
         {
            ...
            if(loader.data.command == Commands.GET_WIFI_STATE)
            {
               if(!loader.data.payload || loader.data.payload.length < 1)
               {
                  Debug.warning("[WifiSettingsScreen] received a wifi state package without payload! mcp:\n" + loader.data);
               ...
               _loc3_ = WifiStates.NAMES[_loc2_];
               lblWifiState.text = Lang.getString("WIFI_STATE_" + _loc3_);
               icnWifiState.removeAllElements();
               if((_loc2_ & 128) == 128)
               {
                  icnWifiState.styleName = "failed";
                  _loc4_ = new ImgNegative();
                  _loc4_.percentHeight = 100;
                  _loc4_.percentWidth = 100;
               }
               else if((_loc2_ & 64) == 64)
               {
                  icnWifiState.styleName = "busy";
                  _loc4_ = new BusyIndicator();
                  _loc4_.percentHeight = 80;
                  _loc4_.percentWidth = 80;
               }
               else if(_loc2_ == WifiStates.CONNECTED)
               {
                  icnWifiState.styleName = "connected";
                  _loc4_ = new ImgPositive();
                  _loc4_.percentHeight = 100;
                  _loc4_.percentWidth = 100;
               ...
               icnWifiState.addElement(_loc4_);
            }
            else if(loader.data.command == Commands.ERROR)
            {
               Debug.warning("[WifiSettingsScreen] requesting wifi state failed (MCP Error)! mcp:\n" + loader.data);
            }
            else
            {
               Debug.warning("[WifiSettingsScreen] requesting wifi state failed (unexpected MCP)! mcp:\n" + loader.data);
            }
            Logicware.finalizeMCPLoader(loader,loaderComplete,loaderFailed);
         },loaderFailed = function(param1:Event):void
         {
            Debug.warning("[WifiSettingsScreen] requesting wifiState failed!\n" + param1);
            InfoCenter.onNetTimeout();
            Logicware.finalizeMCPLoader(loader,loaderComplete,loaderFailed);
         });
         loader.request(MCPBuilder.buildMCP(Commands.GET_WIFI_STATE));
      }
      ...
      private function readNetworkData() : void
      {
         ...
         HoermannRemote.loadBox.title = Lang.getString("WIFI_LOADING_DATA_TITLE");
         HoermannRemote.loadBox.contentText = Lang.getString("WIFI_LOADING_DATA_CONTENT");
         ...
         loader.addEventListener(IOErrorEvent.IO_ERROR,onError = function(param1:Event):void
         {
            loader.removeEventListener(Event.COMPLETE,onComplete);
            loader.removeEventListener(IOErrorEvent.IO_ERROR,onError);
            Debug.error("[WifiSettingsScreen] Could not load http data\n" + param1);
            HoermannRemote.loadBox.close();
         });
      ...
      private function onSelectWifi() : void
      {
         ...
         var loaderFailed:Function = null;
         var loader:MCPLoader = null;
         HoermannRemote.loadBox.title = Lang.getString("WIFI_SCANNING_TITLE");
         HoermannRemote.loadBox.contentText = Lang.getString("WIFI_SCANNING_CONTENT");
         HoermannRemote.loadBox.open(null);
         ...
         this.scannedWifis = new Array();
         loader = Logicware.initMCPLoader(HoermannRemote.appData.activeConnection,loaderComplete = function(param1:Event):void
         {
            timeoutTimer.reset();
            HoermannRemote.appData.activeConnection.processor.removeEventListener(MCPEvent.RECEIVED,onMCP);
            HoermannRemote.loadBox.close();
            if(loader.data.command == Commands.SCAN_WIFI && loader.data.response)
            {
               showWifis();
            }
            else if(loader.data.command == Commands.ERROR)
            {
               Debug.warning("[WifiSettingsScreen] scan_wifi failed (error)! mcp:\n" + loader.data);
            }
            else
            {
               Debug.warning("[WifiSettingsScreen] scan_wifi failed (unexpected MCP)! mcp:\n" + loader.data);
            }
            Logicware.finalizeMCPLoader(loader,loaderComplete,loaderFailed);
         },loaderFailed = function(param1:Event):void
         {
            Debug.warning("[WifiSettingsScreen] requesting mcp scan_wifi failed!\n" + param1);
            InfoCenter.onNetTimeout();
            Logicware.finalizeMCPLoader(loader,loaderComplete,loaderFailed);
         });
         HoermannRemote.appData.activeConnection.processor.addEventListener(MCPEvent.RECEIVED,this.onMCP);
         loader.request(MCPBuilder.buildMCP(Commands.SCAN_WIFI));
      }
      ...
      {
         return "http://" + HoermannRemote.appData.activeConnection.host + "/index.htm";
      }
      ...
      private function onMCP(param1:MCPEvent) : void
      {
         ...
         var _loc4_:int = 0;
         Debug.debug("[WifiSettingScreen] received MCP");
         if(param1.mcp.command != Commands.WIFI_FOUND && param1.mcp.command != Commands.ERROR)
         {
            return;
         }
         Debug.debug("[WifiSettingScreen] MCP is WIFI found");
         if(param1.mcp.command == Commands.WIFI_FOUND)
         {
            _loc2_ = param1.mcp.payload.readUnsignedByte();
            _loc3_ = param1.mcp.payload.readUTFBytes(param1.mcp.payload.bytesAvailable);
            this.scannedWifis.push(_loc3_);
            Debug.debug("[WifiSettingScreen] ADD WIFI");
         }
         else if(param1.mcp.command == Commands.ERROR)
         {
            Debug.debug("[WifiSettingScreen] received MCP ERROR");
            if(!param1.mcp.payload || param1.mcp.payload.length < 1)
            {
               Debug.error("[WifiSettingsScreen] Received MCP Error without payload!\n" + param1.mcp);
               return;
            }
            param1.mcp.payload.position = 0;
            _loc4_ = param1.mcp.payload.readUnsignedByte();
            if(_loc4_ == Errors.GATEWAY_BUSY)
            {
               this.timeoutTimer.reset();
               HoermannRemote.appData.activeConnection.processor.removeEventListener(MCPEvent.RECEIVED,this.onMCP);
               HoermannRemote.loadBox.close();
               Debug.debug("[WifiSettingsScreen] Gateway is already Scanning ...");
            }
         }
      }
      ...
      private function showWifis() : void
      {
         ...
         if(this.scannedWifis == null || this.scannedWifis.length <= 0)
         {
            ...
            HoermannRemote.errorBox.contentText = Lang.getString("WIFI_STATE_AP_NOT_FOUND");
      ...
      private function onSave() : void
      {
         ...
         loader.addEventListener(Event.COMPLETE,onComplete = function(param1:Event):void
         {
            loader.removeEventListener(Event.COMPLETE,onComplete);
            loader.removeEventListener(IOErrorEvent.IO_ERROR,onError);
            Debug.debug("[WifiSettingsScreen] Save complete");
         });
         loader.addEventListener(IOErrorEvent.IO_ERROR,onError = function(param1:Event):void
         {
            loader.removeEventListener(Event.COMPLETE,onComplete);
            loader.removeEventListener(IOErrorEvent.IO_ERROR,onError);
            Debug.debug("[WifiSettingsScreen] Saveing failed\n" + param1);
         });
         loader.load(request);
      }
