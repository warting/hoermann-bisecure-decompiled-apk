package com.isisic.remote.hoermann.views.options.settings
{
   ...
   import com.isisic.remote.hoermann.components.overlays.screens.ResetLoginScreenOverlay;
   ...
   public class ResetLoginScreen extends View implements IBindingClient
   {
      ...
      public function ResetLoginScreen()
      {
         ...
         {
            watcherSetupUtilClass = getDefinitionByName("_com_isisic_remote_hoermann_views_options_settings_ResetLoginScreenWatcherSetupUtil");
         ...
         this.addEventListener("initialize",this.___ResetLoginScreen_View1_initialize);
         var i:uint = 0;
      ...
      private function onInit() : void
      {
         ...
         {
            new ResetLoginScreenOverlay(btnSubmit,btnCancel,bbar.callout).open(null);
         });
      }
      ...
      private function onSubmit(param1:MouseEvent) : void
      {
         ...
         var _loc2_:Object = HoermannRemote.appData.activeGateway;
         if(!_loc2_)
         {
            Debug.error("Can not reset userData! (Gateway not found)");
            return;
         }
         var _loc4_:Boolean = false;
         for each(_loc5_ in _loc2_.users)
         {
            if(_loc5_.id == HoermannRemote.appData.userId)
            {
               _loc3_ = _loc5_;
               _loc4_ = true;
            }
         ...
         if(!_loc4_)
         {
            Debug.error("Can not reset userData! (user not found)");
            _loc6_ = new User();
            _loc6_.id = -1;
            _loc6_.name = HoermannRemote.appData.username;
            _loc6_.scenarios = new Array();
            _loc3_ = _loc6_;
         }
         _loc3_.password = null;
      ...
      public function ___ResetLoginScreen_View1_initialize(param1:FlexEvent) : void
      {
         this.onInit();
      }
