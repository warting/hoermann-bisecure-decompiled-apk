package com.isisic.remote.hoermann.components
{
   ...
   import com.isisic.remote.hoermann.components.popups.AppInfoBox;
   import com.isisic.remote.hoermann.components.popups.InfoBox;
   ...
   import com.isisic.remote.hoermann.views.notifications.NotificationIcon;
   ...
   public class BottomBar extends ActionBar
   {
      ...
      private var coAbout:Button;
      
      private var coNetDebug:Button;
      ...
      override protected function createChildren() : void
      {
         ...
         this.logo = MultiDevice.getFxg(ImgWM);
         if(Features.showDebugLabel)
         {
            debugLabel = new Label();
            debugLabel.percentHeight = 100;
            debugLabel.setStyle("color",13421772);
            this.navigationContent = new Array(this.sp1,this.logo,debugLabel);
         }
         else
         {
            this.navigationContent = new Array(this.sp1,this.logo);
         }
         this.help = new Button();
         this.help.setStyle("icon",MultiDevice.getFxg(ImgHelp));
         ...
         this.calloutNotifier = new NotificationIcon(MultiDevice.getFxg(ImgMenu));
         ...
         this.coHelp = new Button();
         this.coHelp.label = Lang.getString("OPTIONS_HELP");
         this.coHelp.percentWidth = 100;
         this.coHelp.addEventListener(MouseEvent.CLICK,function(param1:MouseEvent):void
         {
            if(HoermannRemote.app.navigator)
            {
               ...
               HoermannRemote.app.navigator.pushView(HelpScreen);
         ...
         this.coNotifications.addEventListener(MouseEvent.CLICK,function(param1:MouseEvent):void
         {
            ...
            HoermannRemote.app.navigator.pushView(NotificationCenter);
         ...
         grp.addElement(this.coHelp);
         grp.addElement(this.coNotifications);
         this.coAbout = new Button();
         this.coAbout.label = "Details";
         this.coAbout.percentWidth = 100;
         this.coAbout.addEventListener(MouseEvent.CLICK,this.onAboutClick);
         if(Features.netDebugging)
         {
            grp.addElement(this.coAbout);
         }
         this.coNetDebug = new Button();
         this.coNetDebug.label = "Net Log";
         this.coNetDebug.percentWidth = 100;
         this.coNetDebug.addEventListener(MouseEvent.CLICK,function(param1:MouseEvent):void
         {
            new SetDebugInfoBox().open(null);
         });
         if(Features.netDebugging)
         {
            grp.addElement(this.coNetDebug);
         }
         var container:Group = new Group();
         container.styleName = HoermannRemote.app.styleName;
         container.addElement(grp);
         this.callout.calloutContent = new Array(container);
         this.refresh = new Button();
         this.refresh.setStyle("icon",MultiDevice.getFxg(ImgRefresh));
         ...
         this.email.setStyle("icon",MultiDevice.getFxg(ImgMail));
