package refactor.logicware._4_HAL.network.tcp.extensions
{
   ...
   public class MCPReaderExtension implements IClientExtension
   {
         ...
         while(_loc1_)
         {
            ...
            else
            {
               ...
               if(this.client.buffer.bytesAvailable >= _loc4_ * MCP.BYTE_MULTIPLIER + 2)
               {
                  ...
                  _loc5_.position = 0;
                  this.client.dispatchEvent(new ClientEvent(ClientEvent.RECEIVE,false,false,_loc5_));
                  _loc5_.position = 0;
                  if(this.client == null || this.client.buffer == null)
                  {
                     Log.info("[MCPReaderExtension] Client seems to be disposed => leaving reader method");
                     return;
                  }
      ...
      {
         Log.warning("Data Completion timeout! (Client waits not longer for protocol completion => clearing receive buffer)");
         this.client.buffer.position = 0;
         Log.debug("buffer:" + this.client.buffer.readUTFBytes(this.client.buffer.bytesAvailable));
