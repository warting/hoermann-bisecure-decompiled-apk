package refactor.bisecur._1_APP.components.popups.portal
{
   ...
   public class SetPortalCredentialsBox extends Popup
   {
      ...
      private var settings:AppSettingDAO;
      
      public function SetPortalCredentialsBox()
      {
         super();
         this.title = Lang.getString("PORTAL_SET_ID_DIALOG");
         this.settings = DAOFactory.getAppSettingDAO();
      }
      ...
      override public function open(param1:DisplayObjectContainer, param2:Boolean = false) : void
      {
         ...
         var _loc3_:PortalCredentials = this.settings.getPortalCredentials();
         if(!_loc3_)
         {
            return;
         }
         this.deviceId = _loc3_.clientId;
         this.password = _loc3_.password;
      }
      ...
      protected function onSubmit(param1:MouseEvent) : void
      {
         ...
         if(this.deviceId == "")
         {
            this.settings.setPortalCredentials(null,null);
            this.close();
            return;
         }
         if(!StringValidator.checkDeviceID(this.deviceId))
         {
            Debug.debug("[SetPortalCredentialsBox] validation failed (Invalide ID)");
            _loc2_ = ErrorBox.sharedBox;
            _loc2_.title = Lang.getString("PORTAL_AUTHENTICATION_FAILED");
            _loc2_.contentText = Lang.getString("PORTAL_AUTHENTICATION_FAILED_CONTENT");
            _loc2_.closeable = true;
            _loc2_.closeTitle = Lang.getString("GENERAL_SUBMIT");
            _loc2_.open(null);
            return;
         }
         Debug.debug("[SetPortalCredentialsBox] Valid ID");
         this.checkUser();
      }
      ...
      {
         var pdata:PortalCredentials = null;
         var self:SetPortalCredentialsBox = null;
         var val:Function = null;
         var fail:Function = null;
         var auth:Function = null;
         pdata = new PortalCredentials();
         pdata.clientId = this.deviceId;
         ...
         c.addEventListener(PortalDataValidateEvent.VALIDATED,val = function(param1:Event):void
         {
            ...
            settings.setPortalCredentials(pdata.clientId,pdata.password);
            Logicware.API.clientId = pdata.clientId;
            errorBox = ErrorBox.sharedBox;
            errorBox.title = Lang.getString("PORTAL_VALIDATION_SUCCEEDED");
            errorBox.contentText = Lang.getString("PORTAL_VALIDATION_SUCCEEDED_CONTENT");
            errorBox.closeable = true;
            errorBox.closeTitle = Lang.getString("GENERAL_SUBMIT");
            errorBox.addEventListener(PopUpEvent.CLOSE,onInfoClose = function(param1:Event):void
            {
               errorBox.removeEventListener(PopUpEvent.CLOSE,onInfoClose);
         ...
         c.addEventListener(PortalDataValidateEvent.VALIDATION_FAILED,fail = function(param1:Event):void
         {
            ...
            var _loc2_:* = ErrorBox.sharedBox;
            _loc2_.title = Lang.getString("PORTAL_VALIDATION_FAILED");
            _loc2_.contentText = Lang.getString("PORTAL_VALIDATION_FAILED_CONTENT");
            _loc2_.closeable = true;
            _loc2_.closeTitle = Lang.getString("GENERAL_SUBMIT");
            _loc2_.open(null);
            changeInputState(true);
         ...
         c.validatePortalData({
            "deviceId":pdata.clientId,
