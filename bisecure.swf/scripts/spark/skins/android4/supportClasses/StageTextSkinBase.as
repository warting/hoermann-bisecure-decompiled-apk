package spark.skins.android4.supportClasses
{
   ...
   public class StageTextSkinBase extends MobileSkin
   {
      ...
      override protected function drawBackground(param1:Number, param2:Number) : void
      {
         ...
         var _loc6_:uint = !!this.isFocused?uint(this.layoutBorderSize + 1):uint(this.layoutBorderSize);
         if(isNaN(_loc4_))
         {
            _loc4_ = 1;
         }
         var _loc7_:int = this.flatheight * 2;
         if(getStyle("contentBackgroundBorder") == "flat")
         {
            graphics.beginFill(_loc3_,_loc4_);
            graphics.drawRect(0,0,param1,param2 - this.flatheight);
            graphics.endFill();
            graphics.beginFill(_loc5_,1);
            graphics.drawRect(0,param2 - _loc7_,_loc6_,this.flatheight);
            graphics.drawRect(0,param2 - this.flatheight,param1,_loc6_);
            graphics.drawRect(param1 - _loc6_,param2 - _loc7_,_loc6_,this.flatheight);
            graphics.endFill();
         }
         else if(getStyle("contentBackgroundBorder") == "rectangle")
         {
            _loc8_ = this.layoutBorderSize * 2;
            graphics.lineStyle(_loc6_,_loc5_,1);
            graphics.beginFill(_loc3_,_loc4_);
            graphics.drawRect(this.layoutBorderSize,this.layoutBorderSize,param1 - _loc8_,param2 - _loc8_);
            graphics.endFill();
         }
      }
